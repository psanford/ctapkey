...
func New(ctx context.Context, uhidName string, opts ...Option) (*SoftToken, error) {
	d, err := uhid.NewDevice(uhidName, rdesc)
	if err != nil {
		return nil, err
	}
...
type SoftToken struct {
	// should emit ctap2 events
	ctap2 bool

	device    *uhid.Device
	evtChan   chan uhid.Event
	authEvent chan HIDEvent
}
...
func (t *SoftToken) Events() chan HIDEvent {
	return t.authEvent
}
...
func (t *SoftToken) Run(ctx context.Context) {
	...
	for {
		...
		case CmdMsg, CmdCbor:
			if cmd == CmdCbor && !t.ctap2 {
				// If we're not doing ctap2 send unsuppoted cmd response for cbor msgs.
				// This will cause clients to fall back to u2f compat mode.
...
// see also https://fidoalliance.org/specs/fido-v2.1-rd-20191217/fido-client-to-authenticator-protocol-v2.1-rd-20191217.html#usb-discovery
var rdesc = []byte{
...
func (t *SoftToken) WriteResponse(ctx context.Context, evt HIDEvent, data []byte, status uint16) error {
